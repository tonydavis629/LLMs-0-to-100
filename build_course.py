...
def expand_component(name: str, attrs: dict[str, str], body: str) -> str:
    ...
    if name == "terminal":
        ...
        lines = [
            ...
            f'    <div class="terminal-output{dense}"{maxw}>',
...
def rewrite_asset_paths(md: str, base: str) -> str:
    """Prefix module-local asset references (``images/``, ``media/``) with ``base``.

    The bundled deck lives in ``slides/`` &mdash; one level above its module's
    ``images/`` and ``media/`` folders &mdash; so relative paths authored in the source
    must be rewritten to point back into the module directory (e.g. ``images/a.png``
    becomes ``module_01_introduction/images/a.png``).
    """
    for prefix in ("images/", "media/"):
        md = md.replace(f"]({prefix}", f"]({base}{prefix}")
        md = md.replace(f'src="{prefix}', f'src="{base}{prefix}')
        md = md.replace(f"src='{prefix}", f"src='{base}{prefix}")
    return md
...
def build_module(module_dir: Path) -> Path:
    ...
    # The bundle lands in slides/ but its assets live under slides/<module>/,
    # so every relative asset path gets prefixed with the module folder name.
    asset_base = f"{module_dir.name}/"
    ...
    out_path = module_dir.parent / f"{module_dir.name}.html"
