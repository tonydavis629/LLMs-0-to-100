...
Inside the model, a batch of text is one tensor of shape **(B, seq_len, d_model)**.
...
Sequences run in parallel. They never attend to each other.
+++
**seq_len (tokens)**

One row per token. **Positional encodings** add an order signal along this axis.
+++
**d_model (features)**

One feature vector per token. **Multi-head attention** splits this axis into heads.
:::
