...
The boxes are **states of the model's weights**; the arrows are the **training processes** that move between them. Pretraining turns raw text into a **base model**, which has absorbed the statistical structure of text &mdash; grammar, facts, style, code patterns &mdash; but has **not** been taught to behave like a helpful assistant. Finetuning into an **instruct model** is Module 6.
