...
Every graph is a matrix: entry $(i,j)$ = weight of edge $j \to i$, zero = no edge.
...
A dense layer is the same matrix with every edge present.
...
+++
**Why GPUs?** Each output is an independent dot product. A GPU runs thousands at once. <!-- .element: class="text-lg" style="margin:0;" -->
:::
...
- Published *Perceptrons* (1969): proved single-layer nets cannot compute XOR, parity, or connectedness
- Widely read as proof that neural networks were a dead end
- Funding dried up for over a decade: the first AI winter for connectionism
- Minsky co-founded MIT's AI Lab; championed symbolic AI
- **They knew multi-layer nets could solve XOR.** No one knew how to *train* them.
