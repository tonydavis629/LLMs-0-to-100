...
Open `module_02_perceptrons/exercise.py` and fill in the `NotImplementedError` lines. Run after each step; unfinished functions are skipped. <!-- .element: class="text-lg" -->
...
Each function is mostly written. You fill in **one key line**. <!-- .element: class="text-lg" style="margin-top: 10px;" -->
...
Implement `SGD.step()`: apply the gradients autograd already computed. <!-- .element: class="text-lg" -->
...
The runner trains the same MLP with `torch.optim.SGD`, then with your class. Same gradients; only the update object changes. <!-- .element: class="text-lg" style="margin-top: 15px;" -->
