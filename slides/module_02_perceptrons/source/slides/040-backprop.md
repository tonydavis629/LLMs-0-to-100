...
Chain rule: multiply one local derivative per stage, **loss &rarr; output &rarr; weight**. Then step against the gradient. <!-- .element: class="text-lg" style="text-align:center; color: var(--muted-color); margin-top:10px;" -->
...
How does a deep weight affect the loss? Multiply local derivatives along the path back to it.
...
- Each node needs only its own **local** derivative
- The chain rule stitches them together
- This is why autograd works at any depth
