...
```
+++
**Hint:** sort the document indices (a `range` over `scores`) with `sorted()`, using each index's score as the key, highest first; slice the first k.
+++
**Answer:**
...
```
+++
**Hint:** multiplying by `mask` zeroes the padding rows; sum over the token axis, then divide by the number of real tokens (the sum of `mask`).
+++
**Answer:**
