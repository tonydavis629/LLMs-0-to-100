...
Both are regenerated by `src/make_checkpoints.py`; the evaluation files by
...
credits. On this suite it changes nothing: both models still get 16/16. Tokens are
characters, 8 of the 16 questions have options of identical length (where dividing
by length cannot change the ranking), and in the other 8 the lengths differ by at
most two characters while the correct option leads the best wrong one by at least
2.3 nats of total log-probability. The normalization matters when option lengths
differ a lot, which this suite does not exercise.
...
3. **The average hides the structure.** The single headline number is −33.3, which
   ...
   Step 6 also prints the pooled average over all 50 cases: 84.0% for instruct
   and 66.0% for rl, a gap of 18.0 points instead of 33.3. Pooling weights each
   task by its case count, and `reverse`, the one task the RL model wins, holds
   24 of the 50 cases. Same answers and same metric, but a different protocol
   gives a different headline.
...
- Every step has its own tests in `tests/`, one file per step. They call the
  student's function on small hand-computed inputs, so a metric that prints
  believable numbers on the real models can still be tagged INCORRECT. The
  arithmetic-mean F1 on the "What an INCORRECT Step Looks Like" slide is the
  example: on the one real partial-credit case precision equals recall, so both
  means agree, and only the test with P = 1 and R = 1/3 separates them.
