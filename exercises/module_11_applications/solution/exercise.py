...
def tokenize(text: str) -> list[str]:
    ...
    # ANSWER: .split() with no argument splits on any whitespace run and
...
def inverse_document_frequency(tokenized_docs: list[list[str]]) -> dict[str, float]:
    ...
    # ANSWER: one log(N / df) per term, in a dict comprehension.
...
def tfidf_vector(tokens: list[str], idf: dict[str, float],
    ...
    for term, count in counts.items():
        ...
        # ANSWER: term frequency times inverse document frequency.
...
def cosine_similarity(a: np.ndarray, b: np.ndarray) -> float:
    ...
    # ANSWER: the dot product with both vector lengths divided out.
...
def rank_documents(query_vector: np.ndarray, doc_vectors: np.ndarray,
    ...
    # ANSWER: sort the document indices by score, best first, keep k.
...
def mean_pool(token_vectors: np.ndarray, attention_mask: np.ndarray) -> np.ndarray:
    ...
    # ANSWER: sum the real token vectors, divide by how many there are.
...
def recall_at_k(ranked_ids: list[str], relevant_ids: list[str], k: int) -> float:
    ...
    Returns:
        A score in [0.0, 1.0].
    """
    # ANSWER: count the relevant ids found in the top k, over how many exist.
...
def reciprocal_rank(ranked_ids: list[str], relevant_ids: list[str]) -> float:
    ...
    for position, doc_id in enumerate(ranked_ids, start=1):
        if doc_id in relevant_ids:
            # ANSWER: first place scores 1.0, second 0.5, third 0.33, and so on.
            return 1.0 / position
    return 0.0
