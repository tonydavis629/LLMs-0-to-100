...
Output plots are saved to `module_02_perceptrons/output/`.

## What to implement

Open `src/neuron.py` and fill in each `raise NotImplementedError(...)` line. Each requires only one line of code.
...
- `linear_separable.csv` -- two clusters a single line can separate
- `non_linear_separable.csv` -- XOR-like pattern requiring a nonlinear boundary

## Extra credit

Implement `mlp_gradients()` to compute backpropagation through the hidden layer. The solution in `solution/src/neuron.py` has the full derivation.
